...
`設定 - サードパーティ` で `オペレーターをYituliuから取得` を有効にし、Yituliuの読み取り専用トークンを入力すると、オペレーター認識はエミュレーターに接続してスクリーンショット認識を行わず、Yituliuに保存された育成データを直接読み取るようになります。この場合、エクスポートされるMarkdown/CSVテーブルには、スキルLv、スキル特化、モジュールの列が追加されます。Yituliuからの取得は UI 側で実装されており、デフォルトのスクリーンショット認識は引き続き Core が行います。
...
認識後の在庫データはキャッシュとして保存され、[在庫維持](./depot-maintain.md) タスクや [作戦](./combat.md) の `目標在庫` モードで使用されます。[データ更新](./user-data-update.md) タスクで自動同期することもできます。
...
- 常設イベント
  - 隠密戦線（エンディングとイベント分岐の選択に対応）
- 常設機能
  - イベントショップ交換
  - 緑チケットショップ交換。[商店購入](./store-shopping.md) から周期実行もできます。
  - 黄チケットショップ交換。[商店購入](./store-shopping.md) から周期実行もできます。
  - 生息演算ショップ交換
  - 潜在能力自動強化
  - 不足素材の合成
